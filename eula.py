...
def check_key():
    ...
    """
    EULA_ACCEPTED_REG_KEY = r"SOFTWARE\BYTECODELLM1"
...
def create_title(eula_text):
    ...
    """
    EULA_TITLE1 = "CYBERARK BYTECODELLM\n"
...
def create_buttons(eula_window):
    ...
    def accept_eula():
        """
        if accepted set EULAAccepted value in BYTECODELLM registry key to 1
        """
        EULA_ACCEPTED_REG_KEY = r"SOFTWARE\BYTECODELLM1"
        EULA_ACCEPTED_REG_VALUE = "EULAAccepted"
        key = winreg.CreateKey(winreg.HKEY_CURRENT_USER, EULA_ACCEPTED_REG_KEY)
        ...
        winreg.CloseKey(key)
        eula_window.destroy()
    # Create a button to decline the EULA, close the window, and exit the program
    def decline_eula():
        eula_window.destroy()
        sys.exit("EULA Declined. Exiting program.")

    # Create Decline Button
    accept_button = tk.Button(eula_window, text="Decline", command=decline_eula)
...
def ensure_eula_accepted():
    """
    Ensure the EULA has been accepted. If not, display the EULA window.
    """
    if not check_key():
        create_eula()
